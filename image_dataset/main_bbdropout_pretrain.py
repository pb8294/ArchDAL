...
from distil.utils.bbdrop.nets.cifar_cnn import BBDROPCNN
from distil.utils.bbdrop.modules.bbdropout import BBDropout
from distil.utils.train_helper import data_train

# utils library for training
from utils.training import train_one, train_one_bbdrop
...
def _train(net, device, epoch, loader_tr, optimizer, nw=None, weight=1, opt_for=None, valid_set=None):
        net.train()
        accFinal = 0.
        # Pradeep Bajracharya
        lossFinal = 0.
        ...
        for batch_id, (x, y) in enumerate(loader_tr):
            ...
            kls = net.get_reg().cuda()
            loss = cent + 1./60000 * kls
            loss.backward()

            # clamp gradients, just in case
            # for p in filter(lambda p: p.grad is not None, clf.parameters()): p.grad.data.clamp_(min=-.1, max=.1)
...
nw = None
figbool=False
if args.network == 'bbdropout':
    net = BBDROPCNN(in_channels=nchannel, n_layers=args.truncation_level, filters=args.max_width)
    net.build_gate(BBDropout, {'kl_scale': 1})
    nw = "bbdrop"
net = net.to(device) 
mlt = 5
if args.pretrain == 100:
    ...
    for epoch in range(args.n_epoch):
        ...
        for batch_idx, (x, y) in enumerate(all_loader):
            ...
            embedding_all.append(e.detach().cpu())
        ...
        t_start = time.time()
        accCurrent, lossCurrent, _, _, _ = _train(net, device, epoch, all_loader_pretrain, optimizer, nw=None, weight=1, opt_for=None, valid_set=None)
        t_end = time.time()
        
        print("{}, {}, Training time: {}".format(accCurrent, lossCurrent, t_end - t_start))
        bbdrop_plot_network_mask_base(net, save_loc + "model{}.png".format(epoch), figsize=figbool)#; exit(0)
...
# results/bbdrop_pretrain_test/bbdropout-CIFAR10-random-1000-n_aug/run1/pretrained_CIFAR10_bbdropout_lr_0.003_batch_64_300_final.pickle
# results/bbdrop_pretrain_test/bbdropout-CIFAR10-random-1000-n_aug/run1/pretrained_CIFAR10_bbdropout_lr_0.003_batch_64_500_final.pickle
pretrain_loc = "results/bbdrop_pretrain_test/bbdropout-" + str(args.dataset) + "-random-" + str(budget) + "-" + str(aug_st) + "/" + args.run
args.pretrain_path = pretrain_loc
if args.pretrain == 1:
    net.get_params()
    pretraining_path = '%s/pretrained_%s_%s_lr_%s_batch_%s_%s_final.pickle' % (pretrain_loc, args.dataset, args.network, args.lr, args.batch_size, 300)
...
net = net.to(device)
# net(torch.rand(4, 1, 32, 32).to(device))
# exit()
load_model = False if args["load"] == 0 else True



# oader_te = DataLoader(valid_set, shuffle=False, pin_memory=True, batch_size=1)
# plt.figure()
# for idx, (x, y) in enumerate(oader_te):
...
inputs = torch.randn((2, nchannel, image_dim, image_dim)).to(device)
net(inputs)
# exit(0)
...
if load_model:
    ...
    if nw is not None:
        bbdrop_plot_network_mask_base(net, save_loc + "/model_before_init.png", figsize=figbool)#; exit(0)
    print('here')
    initial_model, _ = dt.train_bbdrop(None, valid_loader=valid_loader, test_loader=test_loader, save_loc=save_loc, save_path=model_directory, nw=nw)
...
strat_logs = save_loc
acc, loss = train_one_bbdrop(full_train_dataset, initial_train_indices, valid_train_indices, test_loader, copy.deepcopy(initial_model), n_rounds, budget, args, nclasses, args["strategy"], strat_logs, save_loc, "model_{}.pt".format(args["strategy"]), nw=nw)
