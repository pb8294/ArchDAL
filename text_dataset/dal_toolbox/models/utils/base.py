...
class BaseModule(L.LightningModule, abc.ABC):
    ...
    def get_representation(self, *args, **kwargs):
        print(self.model)
        kwargs['device'] = 'cuda' if torch.cuda.is_available() else 'cpu'
        if not hasattr(self.model, 'get_representation'):
            raise NotImplementedError('The `get_representations` method is not implemented.')
        return self.model.get_representation(*args, **kwargs)
    
    # @torch.inference_mode()
    def get_representation_input(self, *args, **kwargs):
        # print(self.model)
        kwargs['device'] = 'cuda' if torch.cuda.is_available() else 'cpu'
        if not hasattr(self.model, 'get_representation_input'):
            raise NotImplementedError('The `get_representations` method is not implemented.')
        return self.model.get_representation_input(*args, **kwargs)
