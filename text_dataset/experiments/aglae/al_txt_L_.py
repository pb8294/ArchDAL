...
def main(args):
    ...
    print(data.train_dataset, data.query_dataset, data.test_dataset)
    # exit(0)
    ...
    # pretrain_loc = os.path.join(here, './pretrained_{}_{}_final.pickle'.format(args.dataset.name, args.model.name))
    # args.pretrain_path = pretrain_loc
    # if args.pretrain == 1:
    here = os.path.dirname(os.path.abspath(__file__))
    pretraining_path = os.path.join(here, './pretrained_{}_{}_final.pickle'.format(args.dataset.name, args.model.name))
    ...
    # exit(0)
    # print(model)
    # print(list(model.state_dict().keys())[:-2])
    # ROBERTA
    ...
    ## UNCOMMENT THE FOLLOWING TILL EXIT() TO PRETRAINT THE NETWORK IN UNSUPERVISED MANNER
    # print(model.get_representation(test_dataloader))
    ...
    # reprs, inps, masks = model.get_representation_input(test_dataloader)
    # lists = []
    # for inp in inps:
    #     for x in inp:
    #         lists.append(x)
    ...
    # print(len(lists))
    # print(reprs.shape)#, inp.shape, msk.shape)
    # # exit(0)
    # diction = {
    #     "label": torch.arange(len(lists)),
    #     "input_ids": lists,
    ...
    for i_acq in range(0, args.al_cycle.n_acq + 1):
        ...
        if args.al_cycle.cold_start:
            ...
            model_dict.update(pretrain_w)
            model.load_state_dict(model_dict)
            print("Reset to pretrained dictionary")
